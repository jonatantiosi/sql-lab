...
dataManutencao as 'Data da manutenção' , 
custo as 'Custo da manutenção'
from Manutencao;

select sum(valorTotal) as 'Total arrecadado' 
from Pagamento
where estado = 'Pago'; 
...
select Veiculo.modelo, Veiculo.marca, count(LocacaoVeiculo.idVeiculo) from LocacaoVeiculo
inner join Veiculo on LocacaoVeiculo.idVeiculo = Veiculo.idVeiculo
group by modelo, marca desc; 
